...
为了帮助你系统地完成第七章，并顺畅衔接第八章，建议按以下节奏推进：

1. **通读 7.1 架构设计**：先画出框架分层图，标注 `core / agents / tools` 三层职责。
2. **完成快速开始**：根据“快速开始”小节安装并运行 30 秒体验，确认 LLM 能正常返回结果。
3. **跟进 7.2 LLM 扩展**：理解多 provider、本地模型与自动检测的设计动机，记录你新增/修改的配置项。
4. **补齐 7.3 核心接口**：逐步实现消息系统、配置管理与异常体系，确保后续 Agent 能稳定调用。
5. **完成 7.4 Agent 范式**：实现 Simple / ReAct / Reflection / Plan-and-Solve，并用示例对话验证行为一致性。
6. **进入 7.5 工具系统**：搭建工具注册、工具链与异步执行器，尝试注册 `calculator` 或 `search` 内置工具。
7. **阅读 7.6 总结**：整理“框架基建清单”，作为第八章记忆与 RAG 扩展的起点。
