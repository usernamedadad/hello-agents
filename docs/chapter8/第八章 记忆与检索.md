...
1. **通读 8.1 理论基础**：理解记忆层次结构与 RAG 的必要性，并对照图 8.2 梳理模块关系。
2. **完成快速体验配置**：按“快速开始”安装依赖与模型，并准备 `.env` 中的向量/图数据库配置（或选择本地方案）。
3. **落实 8.2 记忆系统**：依次实现 `MemoryItem / MemoryConfig / MemoryManager` 等核心组件，验证工作记忆与情景记忆的行为。
4. **实现 8.3 RAG 系统**：完成文档处理、嵌入服务与向量存储的串联，确保检索结果可被 Agent 使用。
5. **复现 8.4 智能问答助手**：用小规模文档集跑通完整流程，检查回答的上下文引用是否合理。
6. **阅读 8.5 总结与展望**：记录可复用的记忆/RAG组件，为第九章上下文工程做准备。
